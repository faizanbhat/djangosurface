  ...
  Drupal.behaviors.postSocial =  {
    attach: function (context, settings) {
      ...
      if (context == document) {
        ...
        var shareImage = $('img', context).attr('src');
      }
      
      var updateWidget = function(){
        // Only proceed on nodes that have the widget.
        if (typeof post_widget != 'undefined') {
          /* I GIVE UP AND I HATE THIS THING.
          // Update all of the configs with the full URL
          // otherwise the tool will always share only the base URL.
          post_widget(".postButtons").updateConfig({
            url: window.location.href,
            image: shareImage,
          });
          // Custom icon - for popups sharing.
          post_widget(".customIcon").updateConfig({
            url: window.location.href,
            image: shareImage,
          });
          */
      ...
      $('body').once('postSocial', function() {
        ...
        // var imagePage = $("meta[property=og\\:image]").attr('content');
      ...
      // We have to init the post social to ensure it's grabbing
      // the correct URL and image.
      // updateWidget();
    }
  }
